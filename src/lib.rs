//! A MiniSAT 2 based SAT solver, usable as a library.
//!
//! # Example
//!
//! ```
...
/// Common types and traits for building and solving SAT problems.
///
/// Importing the prelude brings the core types ([`Lit`], [`Clause`],
/// [`SolverState`], …) and the extension traits whose methods you need to
...
/// ```
pub mod prelude {
    pub use crate::functions::new_clause::NewClause;
    pub use crate::functions::simplify_db::Simplify;
    pub use crate::functions::solve::Solver;
...
/// Parse a problem in DIMACS CNF format and solve it.
///
/// Returns the resulting [`SolverState`]; inspect [`SolverState::ok`] for
/// satisfiability and [`SolverState::solver_stats`] for solver statistics.
pub fn solve_dimacs(buffer: &str) -> SolverState {
...
mod tests {
    ...
    fn assert_model_satisfies(problem: &str) {
        ...
        assert!(!state.model.is_empty(), "model was not populated");
        ...
        for line in problem.lines() {
            ...
            let satisfied = lits.iter().any(|&dimacs_lit| {
                let var = dimacs_lit.abs() as usize - 1;
                match state.model[var] {
                    Lbool::True => dimacs_lit > 0,
                    Lbool::False => dimacs_lit < 0,
                    _ => false,
                }
            });
            assert!(satisfied, "clause not satisfied by model: {line}");
        }
    }
